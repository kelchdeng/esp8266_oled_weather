...
void JDForecast::updateForecastsById(JDForecastData *data,JDForecastData *nowData, String appId, String cityUrl, uint8_t maxForecasts) {
  this->maxForecasts = maxForecasts;
  doUpdate(data,nowData, buildUrl(appId, cityUrl));
}
...
String JDForecast::getCity(String url){
  ...
  if (httpCode > 0)
  {
    ...
    if (httpCode == HTTP_CODE_OK)
    {
      ...
      Serial.println("cityJson："+payload);
